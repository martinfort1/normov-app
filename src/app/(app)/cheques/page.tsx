...
export default async function Cheques() {
  ...
  const { data, error } = await supabase
    .from('v_cheques_panel').select('*').neq('semaforo', 'cerrado')
    .order('vencimiento', { ascending: true, nullsFirst: false }).limit(1000);
      ...
      <div className="panel">
        <table>
          ...
          <tbody>
            ...
            {!rows.length && <tr><td colSpan={9} className="muted">No hay cheques activos.</td></tr>}
          </tbody>
        </table>
      </div>
    </>
